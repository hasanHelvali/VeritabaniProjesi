...
export class GezilenYerlerComponent implements OnInit {
  ...
  getPlaces(){
    ...
    this.http.get<Sehir[]>(this.url).subscribe((result)=>{
      ...
      this.mekanListesi.forEach((mekan:Mekan)=>{
        if(mekan.categoryId==2)this.categoryId2_Mekanlar.push(mekan);
      })
    });
  }
  favoriyeEkle(gelenMekan:Mekan){
    ...
    this.http.put(this.url2+"/"+firebaseKey+"/mekanlar/"+gelenMekan.name.replaceAll(" ","")+"/categoryId/.json",3).subscribe((result)=>{
      console.log(result)
    })
